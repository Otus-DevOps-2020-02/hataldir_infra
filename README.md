...
Домашнее задание № 11 или концерт для бубна с оркестром.
...
GCE.
Роль db перенесена в отдельный личный репозиторий на гитхабе (hataldir/db). В requirements.yml внесено ее описание для скачивания ansible-galaxy. В db.yml изменено имя вызываемой роли, добавлено имя моего аккаунта.
Создан service account в GCE, получен для него credentials-файл в формате json. Аккаунту предоставлен доступ с правами редактора в проект infra-271003.
Установлен драйвер molecule-gce. Переинициализирован сценарий молекулы для db. В плейбук molecule.yml внесены настройки авторизации, изменены плейбуки create и destroy, чтобы получали эти данные из molecule.yml,
а не из переменных окружения.
Выполнены molecule create, converge, verify - машина для теста создалась в GCE, роль накатилась, тесты прошли.

Travis.
Добавлен бейдж, добавлена интеграция со слаком.
В .travis.yml добавлены:
- установка ansible, molecule, apache-libcloud, testinfra
- скачивание, распаковка, установка molecule-gce.
- копирование ключа и credentials в нужные места
- запуск molecule create, converge, verify, destroy

Как хранить в репозитории ключи - пока не придумал. Поэтому репозиторий db приватный.
.travis.yml от роли db добавлен сюда под именем travis.yml.db.example
